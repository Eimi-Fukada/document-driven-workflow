...
## Defect Fix Fast Path

已批准 Feature 内的小缺陷修复可以走快路径，目标是减少重复文档工作，但不降低验收要求。

适用条件：

- 当前工作已经有批准过的 Feature 文档包。
- 缺陷修复仍属于当前 Feature 的 Scope Lock、REQ / AC 和允许文件范围。
- 不新增产品目标、不扩大模块边界、不触碰 forbidden scope。
- 不涉及认证、支付、权限、数据库迁移、生产部署、数据破坏、任务状态一致性或老项目核心兼容边界。

执行方式：

- 不重新 hydrate，不重新拆 Feature，不生成新的 Epic / Feature。
- 开工前只重新锁定当前 `00-workflow.yaml`、Scope Lock、相关 AC / COV 和验证要求。
- 如果缺陷暴露出验收标准缺失，只补充当前 Feature 的验收标准和验证报告，不扩展成新的产品迭代。
- 修改完成后更新 `07-verification-report.md` 或 Light Feature 验证区，记录症状、根因、修复位置和验证证据。
- 最后仍必须通过 `finish-feature.mjs`，不能用“快路径”跳过完成出口。

如果修复需要超出当前 Feature 范围，必须停下来更新 Feature 文档或创建新的 Feature。

## Verification Strategy

验证策略要避免重复跑昂贵全量命令，同时不能降低完成证据。

默认策略：

- 实现过程中优先跑 targeted checks，例如受影响文件 lint、相关单测、局部 typecheck、API smoke 或手工复现路径。
- 不要在实现中先跑一轮昂贵全量验证，然后在 `finish-feature.mjs` 中无意义重复同一类全量验证。
- `finish-feature.mjs` 仍是最终完成出口。提前跑过的验证只能作为证据，不能替代最终完成检查。
- 如果提前跑了昂贵验证，必须把命令、时间、结果和覆盖的 REQ / AC / COV 写入验证报告。

不能复用或弱化验证的情况：

- 验证后又改了相关代码、配置、依赖、脚本、数据结构或验收范围。
- 前面的验证命令与当前完成要求不一致。
- 前面的验证没有覆盖当前 AC / COV。
- 验证证据没有写入验证报告。
- Feature 属于高风险边界，例如认证、支付、权限、数据库迁移、生产部署、数据破坏、任务状态一致性或老项目核心兼容。

现阶段不做机器级自动跳过验证。优先通过执行纪律减少重复；只有真实项目证明重复验证成本很高时，再考虑把证据复用做进脚本。
