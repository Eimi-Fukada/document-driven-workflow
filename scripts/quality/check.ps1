...
$required = @(
    ...
    "docs/workflow/LANGUAGE_POLICY.md",
    ...
    "docs/workflow/presets/flutter-app.md",
    "docs/workflow/templates/feature/00-workflow.yaml",
    "docs/workflow/templates/feature/REVIEW.md",
    ...
    "docs/workflow/templates/epic/REVIEW.md",
...
$languagePolicy = ReadText "docs/workflow/LANGUAGE_POLICY.md"
...
$legacyAdoption = ReadText "docs/workflow/LEGACY_ADOPTION.md"
...
$featureTechnicalTemplate = ReadText "docs/workflow/templates/feature/03-technical-contract.md"
$featurePlanTemplate = ReadText "docs/workflow/templates/feature/06-implementation-plan.md"
$featureVerificationTemplate = ReadText "docs/workflow/templates/feature/07-verification-report.md"
...
$adoptScript = ReadText "scripts/workflow/automation/adopt.mjs"
...
$templateStatusDocs = @()
$templateStatusDocs += ReadText "docs/workflow/templates/feature/REVIEW.md"
$templateStatusDocs += ReadText "docs/workflow/templates/epic/REVIEW.md"
...
$contentChecks = @(
    ...
    @{ Name = "USER_GUIDE.md keeps user entry concise"; Pass = $userGuide -match "REVIEW.md" -and -not ($userGuide -match "completion-check.mjs|handoff-pack.mjs|gate-feature.mjs|gate-epic.mjs") },
    ...
    @{ Name = "EXECUTION_DISCIPLINE.md defines defect fast path and verification strategy"; Pass = $executionDiscipline -match "Defect Fix Fast Path" -and $executionDiscipline -match "Verification Strategy" -and $executionDiscipline -match "targeted checks" -and $executionDiscipline -match "finish-feature.mjs" },
    ...
    @{ Name = "EXECUTION_DISCIPLINE.md defines option decision contract"; Pass = $executionDiscipline -match "Option Decision Contract" -and $executionDiscipline -match "Selected option" -and $executionDiscipline -match "Rejected options" -and $executionDiscipline -match "Fallback options" },
    ...
    @{ Name = "USAGE.md separates user AI and maintainer entries"; Pass = $usage -match "adopt.mjs" -and $usage -match "process.mjs" -and $usage -match "continue.mjs" -and $usage -match "finish-feature.mjs" },
    ...
    @{ Name = "LANGUAGE_POLICY.md defines language split"; Pass = $languagePolicy -match "User-facing generated documents" -and $languagePolicy -match "Machine-readable protocol" -and $languagePolicy -match "English normative rules" },
    ...
    @{ Name = "AUTOMATION.md separates natural language and script entries"; Pass = $automation -match "adopt.mjs" -and $automation -match "process.mjs" -and $automation -match "continue.mjs" -and $automation -match "finish-feature.mjs" -and $automation -match "completion-check.mjs" },
    ...
    @{ Name = "USAGE.md documents document packages"; Pass = $usage -match "REVIEW.md" -and $usage -match "00-source.md" -and $usage -match "01-prd.md" -and $usage -match "01-light-feature.md" },
    ...
    @{ Name = "GATES.md requires review summary"; Pass = $gates -match "Review Summary Gate" -and $gates -match "REVIEW.md" -and $gates -match "not approval" },
    ...
    @{ Name = "GATES.md documents option deviation gate"; Pass = $gates -match "Option Deviation Gate" -and $gates -match "Selected option" -and $gates -match "fallback option" },
    ...
    @{ Name = "STACK_POLICY.md defines built-in and custom stacks"; Pass = $stackPolicy -match "next-fullstack" -and $stackPolicy -match "flutter-app" -and $stackPolicy -match "legacy-existing" -and $stackPolicy -match "custom stack" },
    @{ Name = "LEGACY_ADOPTION.md defines adoption levels"; Pass = $legacyAdoption -match "Minimal" -and $legacyAdoption -match "Standard" -and $legacyAdoption -match "Strict" -and $legacyAdoption -match "legacy-existing" },
    ...
    @{ Name = "Skill mentions language policy"; Pass = $skill -match "LANGUAGE_POLICY.md" -and $skill -match "Machine-readable protocol" },
    @{ Name = "Skill maintainer notes are not replacement-corrupted"; Pass = $skill -match "zh:" -and -not ($skill.Contains([string][char]0xFFFD)) },
    ...
    @{ Name = "Skill mentions single Feature closure"; Pass = $skill -match "Feature" -and $skill -match "finish-feature.mjs" -and $skill -match "free implementation" },
    @{ Name = "Skill mentions defect fast path and duplicate verification avoidance"; Pass = $skill -match "defect-fix fast path" -and $skill -match "duplicate expensive verification" -and $skill -match "targeted checks" },
    ...
    @{ Name = "Coverage matrix requires changed-file evidence"; Pass = $coverageMatrixScript -match "changedFilesEvidence" -and $featureVerificationTemplate -match "Changed Files Evidence" -and $featureVerificationTemplate -match "Implementation Evidence" },
    @{ Name = "Adopt script writes concise persistent rules"; Pass = $adoptScript -match "Hard rules" -and $adoptScript -match "finish-feature.mjs" -and $adoptScript -match "COMPLETION_PROOF.json" -and $adoptScript -match "package.json" },
    @{ Name = "Gate subject does not keep legacy path options"; Pass = -not ($gateSubjectScript -match "FeaturePath|EpicPath") },
    @{ Name = "Feature scripts support light mode"; Pass = $featureNewScript -match "--mode" -and $featureNewScript -match '"REVIEW.md", "01-light-feature.md"' -and $gateSubjectScript -match "01-light-feature.md" },
    @{ Name = "Gate subject requires review summary"; Pass = $gateSubjectScript -match "checkReviewSummary" -and $gateSubjectScript -match '"REVIEW.md"' -and $gateSubjectScript -match "Selected option" },
    ...
    @{ Name = "Doctor supports common non-Node project markers"; Pass = $doctorScript -match "pom.xml" -and $doctorScript -match "build.gradle" -and $doctorScript -match "go.mod" -and $doctorScript -match "csproj" },
    ...
    @{ Name = "Automation scripts check option decisions"; Pass = $completionCheckScript -match "Rejected/fallback override evidence" -and $completionCheckScript -match "Selected option" -and $handoffPackScript -match "option_decision" },
    ...
    @{ Name = "Handoff pack carries option boundaries"; Pass = $handoffPackScript -match "selected_option" -and $handoffPackScript -match "rejected_options" -and $handoffPackScript -match "fallback_options" },
    @{ Name = "Feature templates define option IDs and override fields"; Pass = $featureTechnicalTemplate -match "OPT-001" -and $featurePlanTemplate -match "Selected option" -and $featurePlanTemplate -match "User override required" -and $featureVerificationTemplate -match "Rejected option used" },
    @{ Name = "Feature templates define defect fast path and verification strategy fields"; Pass = $featurePlanTemplate -match "Defect fix fast path" -and $featurePlanTemplate -match "Verification strategy" -and $featureVerificationTemplate -match "Verification Strategy Notes" -and $featureVerificationTemplate -match "Duplicate full verification avoided" },
